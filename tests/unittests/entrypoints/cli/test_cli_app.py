...
def test_cli_fail_when_outpu_not_exists():
    result = runner.invoke(app, ["rc", "-o", "not_exists.xml"], obj={})
...
def test_cli_fail_when_config_not_exists():
    result = runner.invoke(
        app, ["rc", "-o", str(existing_xml_path), "-c", "config_not_exists"], obj={}
    )
    assert result.exit_code == 2
...
def test_cli_by_default_uses_include_passing_false():
    result = runner.invoke(app, ["rc", "-o", str(existing_xml_path)], obj={})
    assert result.exit_code == 0
    assert "Include Passing: False" in result.output


def test_cli_sets_include_passing():
    result = runner.invoke(app, ["rc", "-o", str(existing_xml_path), "-i"], obj={})
    assert result.exit_code == 0
...
def test_cli_stets_generating_report():
    result = runner.invoke(
        app, ["rc", "-o", str(existing_xml_path), "-r", "report.html"], obj={}
    )
    assert result.exit_code == 0
...
def test_cli_calls_main_function():
    result = runner.invoke(
        app,
        ["rc", "-o", str(existing_xml_path)],
        obj={"rc": lambda *args: echo("RUNNING MAIN")},
    )
    assert result.exit_code == 0
...
# TODO: add test for installation entrypoint
